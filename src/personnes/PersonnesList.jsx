...
function PersonnesList() {
  ...
  const personnesFuse = new Fuse(personnes, {keys: ["brand", "model", "year", "type"], threshold: 0.3})
  ...
  const deletePersonne = (id) => {
    if (confirm("Are you sure you want to delete this personne")) {
      axios.delete(`http://127.0.0.1:8000/api/personnes/${id}`).then(() => {
        dispatch({ type: "UPDATE_PERSONNES", payload: personnes.filter(c => c.id !== id) });
      });
    };
    }
  ...
  return (
    <div className="border-sh rounded-xl overflow-hidden mx-1 md:mx-4 h-fit my-4">
      <div className="flex flex-wrap justify-between items-center gap-6 my-4 px-3">
        ...
        <button className="join-item btn btn-outline btn-info btn-sm" onClick={handleAdd}><FontAwesomeIcon icon={faPlus} /> New Personne</button>
      </div>
                ...
                <th>
                  <div className="join">
                    ...
                    <button
                      ...
                      onClick={() => handleModify(personne.id)}
                    >
                      <FontAwesomeIcon icon={faPenToSquare} /> Edit
                    </button>
                    <button
                      className="join-item btn btn-outline btn-secondary btn-sm"
                      onClick={() => deletePersonne(personne.id)}
                    >
